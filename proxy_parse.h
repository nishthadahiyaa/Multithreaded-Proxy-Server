 ...
 */
 ...
 /*
         ...
         Comes from the first word in the request line.
         char *protocol; →
         Protocol scheme: "http" or "https".
         Extracted from the URL if present.
         char *host; →
         The domain: "www.google.com".
         Either from the URL or from the Host: header.
         char *port; →
         Port number as a string.
         Example: "80" (HTTP default) or "443" (HTTPS).
         If no explicit :port in the URL, it uses the default.
         char *path; →
         Path and query string part of the URL:
         e.g. "/?client=safari&zx=1757049132468&no_sw_cr=1"
         char *version; →
         HTTP version, e.g. "HTTP/1.1" or "HTTP/2".

         ex: GET https://www.google.com/?client=safari&zx=1757049132468&no_sw_cr=1 HTTP/1.1
 */
...
/* 
   ParsedHeader: any header af ter the request line is a key-value pair with the
